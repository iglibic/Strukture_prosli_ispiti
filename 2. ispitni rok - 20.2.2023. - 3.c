...
// Funkcija za izračun prosječne cijene proizvoda unutar kategorije
void calculateAveragePrice(KategorijaP category) {
    ...
    if (count > 0) {
        category->avgCijena = (float)totalPrice / count;
    }
}

// Funkcija za brisanje proizvoda skupljih od prosječne cijene
void removeExpensiveProducts(KategorijaP category) {
    ProizvodP current = category->child, prev = NULL;
    
    while (current != NULL) {
        if (current->cijena > category->avgCijena) {
            if (prev == NULL) {
                category->child = current->next;
            } else {
                prev->next = current->next;
            }
            ProizvodP temp = current;
            current = current->next;
            free(temp);
        } else {
            prev = current;
...
void printCategories(KategorijaP head) {
    ...
    while (category != NULL) {
        printf("Kategorija: %s (Min: %d, Max: %d, Avg: %.2f)\n", category->imeKategorije, category->minCijena, category->maxCijena, category->avgCijena);
        ProizvodP product = category->child;
        while (product != NULL) {
...
int main() {
    ...
    // Izračunaj prosječnu cijenu za svaku kategoriju
    KategorijaP category = categories;
    while (category != NULL) {
        calculateAveragePrice(category);
        category = category->next;
    }

    // Brisanje proizvoda skupljih od prosječne cijene unutar svake kategorije
    category = categories;
    while (category != NULL) {
        removeExpensiveProducts(category);
        category = category->next;
    }

    printf("Kategorije i proizvodi nakon brisanja skupljih proizvoda:\n");
